...
sub get_param {
    ...
    eval {
        ...
        $self->dbg("Error '$@' was occured while getting property: $param") unless $mute;
        $retval = undef;
    };
...
sub get_params_as_hashref {
    ...
    for my $param_name (@params_list) {
        my $param = $self->get_param($param_name, 1);
        unless(defined $param) {
            $param = $self->get_param("${prefix}_${param_name}")
        }
        next unless defined $param;
        $retval->{$param_name} = trim_input($param);
    }
    for my $key (sort keys %$retval) {
        $self->logger->info(qq{Got parameter "$key" with value "$retval->{$key}"});
    }
    return $retval;
}
